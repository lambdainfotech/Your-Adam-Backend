...
<div class="max-w-2xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
        <div class="flex items-center mb-6">
            ...
            <div>
                <h2 class="text-xl font-bold text-gray-800">Create Customer</h2>
                <p class="text-gray-500">Add a new customer account</p>
            </div>
        </div>
            ...
            <div class="space-y-6">
                ...
                <div>
                    <label for="password" class="block text-sm font-medium text-gray-700 mb-2">
                        Password <span class="text-gray-400 text-xs">(Optional - auto-generated if empty)</span>
                    </label>
                    <input type="password" name="password" id="password"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                        placeholder="Leave blank for auto-generated password">
                    <p class="mt-1 text-xs text-gray-500">If left empty, a random password will be generated and shown after creation</p>
                    @error('password')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="role_id" id="role_id" required
                        ...
                        @foreach($roles as $role)
                            <option value="{{ $role->id }}" {{ old('role_id', $role->name == 'Customer' ? $role->id : '') == $role->id ? 'selected' : '' }}>
                                {{ $role->name }}
                            </option>
                        @endforeach
                    </select>
