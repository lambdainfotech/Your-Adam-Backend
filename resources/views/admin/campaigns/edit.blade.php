...
<div class="bg-white rounded-lg shadow p-6">
    <form action="{{ route('admin.campaigns.update', $campaign) }}" method="POST" enctype="multipart/form-data">
            ...
            <div>
                ...
                <select name="discount_type" required 
                    ...
                    @foreach($discountTypes as $key => $label)
                        <option value="{{ $key }}" {{ old('discount_type', $campaign->discount_type) == $key ? 'selected' : '' }}>
                            {{ $label }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
            ...
            <div>
                ...
                @if($campaign->banner_image)
                    <div class="mb-2">
                        <img src="{{ $campaign->banner_image_url }}" alt="Banner" class="h-20 rounded">
                        <p class="text-xs text-gray-500">Current image</p>
                    </div>
                @endif
                <input type="file" name="banner_image" accept="image/*"
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                <p class="text-xs text-gray-500 mt-1">Upload new image to replace (Max: 2MB)</p>
            </div>
        </div>
        ...
        <div class="mb-6 p-4 bg-gray-50 rounded-lg">
            ...
            <div class="flex flex-wrap gap-4 mb-4">
                <label class="inline-flex items-center">
                    <input type="radio" name="apply_type" value="all" 
                        {{ old('apply_type', $campaign->apply_type) == 'all' ? 'checked' : '' }}
                ...
                <label class="inline-flex items-center">
                    <input type="radio" name="apply_type" value="products" 
                        {{ old('apply_type', $campaign->apply_type) == 'products' ? 'checked' : '' }}
                        class="apply-type-radio w-4 h-4 text-blue-600">
                    <span class="ml-2">Selected Products</span>
                </label>
                <label class="inline-flex items-center">
                    <input type="radio" name="apply_type" value="categories" 
                        {{ old('apply_type', $campaign->apply_type) == 'categories' ? 'checked' : '' }}
                        class="apply-type-radio w-4 h-4 text-blue-600">
            ...
            <div id="productsSection" class="{{ old('apply_type', $campaign->apply_type) == 'products' ? '' : 'hidden' }}">
                    ...
                    @foreach($products as $product)
                        <label class="flex items-center py-1 hover:bg-gray-50">
                            <input type="checkbox" name="selected_products[]" value="{{ $product->id }}"
                                {{ in_array($product->id, old('selected_products', $selectedProducts)) ? 'checked' : '' }}
            ...
            <div id="categoriesSection" class="{{ old('apply_type', $campaign->apply_type) == 'categories' ? '' : 'hidden' }}">
                    ...
                    @foreach($categories as $category)
                        <label class="flex items-center py-1 hover:bg-gray-50">
                            <input type="checkbox" name="selected_categories[]" value="{{ $category->id }}"
                                {{ in_array($category->id, old('selected_categories', $selectedCategories)) ? 'checked' : '' }}
                                class="rounded border-gray-300 text-blue-600 mr-2">
