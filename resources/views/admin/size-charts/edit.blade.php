            ...
            <div>
                ...
                <div class="flex items-center gap-4 mt-2">
                    @foreach($sizeTypes as $key => $label)
                        <label class="inline-flex items-center p-3 border rounded-lg cursor-pointer hover:bg-gray-50 {{ old('size_type', $sizeChart->size_type) == $key ? 'border-blue-500 bg-blue-50' : 'border-gray-300' }}">
                            <input type="radio" name="size_type" value="{{ $key }}" 
                                {{ old('size_type', $sizeChart->size_type) == $key ? 'checked' : '' }}
                                class="w-4 h-4 text-blue-600 border-gray-300 focus:ring-blue-500"
                                onchange="updateSizeTypeStyle(this)">
                            <span class="ml-2 text-sm font-medium {{ old('size_type', $sizeChart->size_type) == $key ? 'text-blue-700' : 'text-gray-700' }}">{{ $label }}</span>
                        </label>
                    @endforeach
                </div>
                @error('size_type')
